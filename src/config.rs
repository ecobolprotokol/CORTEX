...
impl CortexConfig {
    ...
    pub fn validate(&self) -> Result<(), CortexError> {
        ...
        if self.model.sparsity_ratio <= 0.0 || self.model.sparsity_ratio > 1.0 {
            return Err(CortexError::ConfigError("sparsity_ratio must be in (0, 1]".into()));
        }
        if self.language.vocabulary_capacity < 256 {
            return Err(CortexError::ConfigError("vocabulary_capacity must be >= 256".into()));
        }
        if self.language.context_window < 64 {
            return Err(CortexError::ConfigError("context_window must be >= 64".into()));
        }
        if self.language.generation_limit < 32 {
            return Err(CortexError::ConfigError("generation_limit must be >= 32".into()));
        }
        if self.memory.working_mb < 16 {
            return Err(CortexError::ConfigError("working_mb must be >= 16".into()));
        }
        if self.memory.episodic_mb < 32 {
            return Err(CortexError::ConfigError("episodic_mb must be >= 32".into()));
        }
        if self.memory.semantic_mb < 32 {
            return Err(CortexError::ConfigError("semantic_mb must be >= 32".into()));
        }
        if self.learning.learning_rate <= 0.0 || self.learning.learning_rate > 1.0 {
            return Err(CortexError::ConfigError("learning_rate must be in (0, 1]".into()));
        }
        if self.learning.plasticity < 0.0 || self.learning.plasticity > 1.0 {
            return Err(CortexError::ConfigError("plasticity must be in [0, 1]".into()));
        }
        if self.verification.minimum_confidence < 0.0 || self.verification.minimum_confidence > 1.0 {
            return Err(CortexError::ConfigError("minimum_confidence must be in [0, 1]".into()));
        }
        Ok(())
    }
}
