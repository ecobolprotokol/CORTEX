...
impl EvidenceSet {
    ...
    pub fn total_strength(&self) -> Scalar {
        ...
        self.items.iter().map(|e| e.strength).sum::<Scalar>() / self.items.len() as Scalar
    }
    ...
    pub fn derived(parents: &[Provenance]) -> Self {
        Self {
            ...
            evidence: {
                ...
                for p in parents {
                    for item in &p.evidence.items {
                        evidence_set.items.push(item.clone());
                    }
                }
                evidence_set
            },
...
pub struct UncertaintyState {
    pub level: Scalar,
    pub dimensions: std::collections::HashMap<String, Scalar>,
    pub reducible: bool,
